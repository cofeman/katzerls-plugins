...
namespace FCBot.Helpers
{
    internal static class Movement
    {
        ...
        {
            return new Decorator(ret => Setting.Instance.MoveToTarget.Contains("Always") && toUnit != null && toUnit(ret) != null && !toUnit(ret).IsMe && !InLineOfSpellSight(toUnit(ret)),
                new Sequence(
                    new Action(ret => Log.DebugLog("MoveToLoss: moving to LoSS of {0} @ {1:F1} yds", toUnit(ret).SafeName(), toUnit(ret).Distance)),
        ...
        public static Composite CreateEnsureMovementStoppedBehavior(float range = float.MaxValue, UnitSelectionDelegate onUnit = null, string reason = null)
        {
            if (range == float.MaxValue)
            {
                return new Decorator(
                    ret => Setting.Instance.MoveToTarget.Contains("Always") && StyxWoW.Me.IsMoving,
                    new Sequence(
                        new Action(ret => Log.DebugLog("EnsureMovementStopped: stopping! {0}", reason ?? "")),
            ...
            return new Decorator(
                ret => Setting.Instance.MoveToTarget.Contains("Always")
                    && StyxWoW.Me.IsMoving
                    && (onUnit(ret) == null || onUnit(ret).Distance < range),
                new Sequence(
                    new Action(ret => Log.DebugLog("EnsureMovementStopped: stopping because {0}", onUnit(ret) == null ? "No CurrentTarget" : string.Format("target @ {0:F1} yds, stop range: {1:F1}", onUnit(ret).Distance, range))),
                    new Action(ret => StopMoving.Now())
                    )
                );
        }
        ...
        {
            return new Decorator( ret => Setting.Instance.MoveToTarget.Contains("Always") && StyxWoW.Me.IsMoving && InMoveToMeleeStopRange(StyxWoW.Me.CurrentTarget),
                new Sequence(
                    new Action(ret => Log.DebugLog("EnsureMovementStoppedWithinMelee: stopping because {0}", !StyxWoW.Me.GotTarget ? "No CurrentTarget" : string.Format("target at {0:F1} yds", StyxWoW.Me.CurrentTarget.Distance))),
                    new Action(ret => StopMoving.Now())
                    )
                );
        }
        ...
        {
            return new Decorator(
                ret => Setting.Instance.MoveToTarget.Contains("Always") && onUnit != null && onUnit(ret) != null && onUnit(ret).Distance > range,
                new Sequence(
                    new Action(ret => Log.DebugLog("MoveToUnit: moving within {0:F1} yds of {1} @ {2:F1} yds", range, onUnit(ret).SafeName(), onUnit(ret).Distance)),
            ...
            return new Decorator(
                ...
                new PrioritySelector(
                    ...
                    new Decorator(
                        ...
                        new Sequence(
                            new Action(ret => Log.DebugLog("MoveToMelee: towards {0} @ {1:F1} yds", StyxWoW.Me.CurrentTarget.SafeName(), StyxWoW.Me.CurrentTarget.Distance)),
...
#else
            return new Decorator(
                ret => Setting.Instance.MoveToTarget.Contains("Always") && StyxWoW.Me.CurrentTarget != null && !StyxWoW.Me.CurrentTarget.IsWithinMeleeRange,
                new Sequence(
                    new Action(ret =>
                    {
                        MoveResult res = Navigator.MoveTo(StyxWoW.Me.CurrentTarget.Location);
                        Log.DebugLog("MoveToMelee({0}): towards {1} @ {2:F1} yds", res.ToString(), StyxWoW.Me.CurrentTarget.SafeName(), StyxWoW.Me.CurrentTarget.Distance);
                    }),
        ...
        public static bool InMoveToMeleeStopRange(WoWUnit unit)
        {
            ...
            {
                float preferredDistance = unit.MeleeDistance() - 1f;
        ...
        public static Composite CreateMoveBehindTargetBehavior(SimpleBooleanDelegate requirements)
        {
            return new Decorator(ret =>
                {
                    if (Setting.Instance.MoveToTarget.Contains("Never") || !requirements(ret) || StyxWoW.Me.IsCasting || Groups.MeIsTank) return false;
                ...
                new PrioritySelector(ctx => CalculatePointBehindTarget(),
                    new Decorator(req => Navigator.CanNavigateFully(StyxWoW.Me.Location, (WoWPoint)req, 4),
                        new Sequence(
                            new Action(ret => Log.DebugLog("MoveBehind: behind {0} @ {1:F1} yds", StyxWoW.Me.CurrentTarget.SafeName(), StyxWoW.Me.CurrentTarget.Distance)),
        ...
        public static WoWPoint CalculatePointBehindTarget()
        {
            ...
            return StyxWoW.Me.CurrentTarget.Location.RayCast(facing, StyxWoW.Me.CurrentTarget.MeleeRange() - 2f);
        }
    ...
    public static class StopMoving
    {
        ...
        public static void Pulse()
        {
            ...
            if (stopMovingNow)
            {
                if (!StyxWoW.Me.IsMoving)
                    Log.DebugLog("StopMoving: character already stopped, clearing stop {0} request", Type);
                else
                {
        ...
        {
            if (Setting.Instance.MoveToTarget.Contains("Never"))
                return;
        ...
        public static void Now()
        {
            ...
            if (StyxWoW.Me.IsMoving)
            {
                Log.DebugLog("StopMoving.Now: character already stopped, clearing stop {0} request", Type);
                Navigator.PlayerMover.MoveStop();
            }
        }
