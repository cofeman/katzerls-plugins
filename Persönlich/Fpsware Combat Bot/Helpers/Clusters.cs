...
namespace FCBot.Helpers
{
    internal static class Clusters
    {
        ...
        {
            IList<WoWUnit> woWUnits = otherUnits as IList<WoWUnit> ?? otherUnits.ToList();
            if (!woWUnits.Any()) return 0;

            switch (type)
            {
                case ClusterType.Radius:
                    return GetRadiusClusterCount(target, woWUnits, clusterRange);
                
                case ClusterType.Chained:
                    return GetChainedClusterCount(target, woWUnits, clusterRange);
                
                case ClusterType.Cone:
                    return GetConeClusterCount(target, woWUnits, clusterRange);
                default:
        ...
        public static WoWUnit GetBestUnitForCluster(IEnumerable<WoWUnit> units, ClusterType type, float clusterRange)
        {
            ...
            {
                case ClusterType.Radius:
                    return (from u in units select new {Count = GetRadiusClusterCount(u, units, clusterRange), Unit = u}).OrderByDescending(a => a.Count).FirstOrDefault().Unit;
                
                case ClusterType.Chained:
                    return (from u in units select new {Count = GetChainedClusterCount(u, units, clusterRange), Unit = u}).OrderByDescending(a => a.Count).FirstOrDefault().Unit;
        ...
        public static int GetRadiusClusterCount(WoWUnit target, IEnumerable<WoWUnit> otherUnits, float radius)
        {
            var targetLoc = target.Location;
        ...
        {
            IEnumerable<int> unitCounters = otherUnits.Select(u => GetUnitsChainWillJumpTo(target, otherUnits.ToList(), chainRange).Count);
